  ...
  try {
    ...
    try {
      ...
      if (!isDevStoreFallbackAllowed()) {
        console.error('[Wholesale Fulfill] Database error, no fallback in this environment:', dbErr);
        return NextResponse.json(
          { error: 'A server error occurred. Please try again.' },
          { status: 500 }
        );
      }
      console.warn('[Wholesale Fulfill] Database offline, using devStore fallback (non-production only):', dbErr.message);
