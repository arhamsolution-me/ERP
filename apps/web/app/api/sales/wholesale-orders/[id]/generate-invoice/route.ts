  ...
  try {
    ...
    try {
      ...
      if (!isDevStoreFallbackAllowed()) {
        console.error('[Wholesale Invoice] Database error, no fallback in this environment:', dbErr);
        return NextResponse.json(
          { error: 'A server error occurred. Please try again.' },
          { status: 500 }
        );
      }
      console.warn('[Wholesale Invoice] Database offline, using devStore fallback (non-production only):', dbErr.message);
